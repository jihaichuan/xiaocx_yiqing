// pages/index/residential_quarters.js
var app = getApp();
Page({
     ...
     * 打印进入登记码
     */
    down_qrcode2: function () {
        var that = this;
        if (that.data.info.qrcode_url2 == '') {
            app.show_loading('加载中');
            that.set_qrcode(function (res) {
                app.basic_dialog('操作结果', '登记码生成成功，请进入下载', '去下载', function () {
                    that.go_qrcode(res.qrcode_url2);
                })
            });
        } else {
            that.go_qrcode(that.data.info.qrcode_url2);
        }
    },
